"""MERT-R4 discovery probes (copy of r4/r4_probes.py, featsmert input mode).
Original: R4 discovery probes (the point of the model). Usage: r4_probes.py <tag>
...
from mert_r4_model import R4Conditioned, _rope_cache

TAG = sys.argv[1]
DEVICE = "cuda:1"; FPS = 44100 / 1024
cache = torch.load("/disk4/jaehoon/VBPM_cache/mert/cache_mert_fold0_c1400.pt", weights_only=False)
ckpt = torch.load(HERE / f"runs/mertr4_{TAG}.pt", weights_only=False)
INPUT = ckpt["input"]
model = R4Conditioned(fps=FPS, input_mode=INPUT, device=DEVICE, input_dim=ckpt.get("input_dim"))
...
def trunk_input(e):
    ...
    if INPUT == "feats":
        return f
    m = torch.from_numpy(cache["val_mert"][e["stem"]].astype(np.float32)).to(DEVICE)
    return torch.cat([f, (m - mmean) / mstd], dim=1)
...
json.dump(report, open(HERE / f"results_mertprobes_{TAG}.json", "w"), indent=1, default=float)
