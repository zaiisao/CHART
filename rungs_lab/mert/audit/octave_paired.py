...
common=sorted(set(R["MERT_SHORT150"])&set(R["BT_SHORT150"])&set(R["BTsup_long"])&set(R["BTunsup_long"]))
import numpy as np
from scipy.stats import wilcoxon
def arr(k): return np.array([R[k][s] for s in common])
pairs=[("MERT_SHORT150","BT_SHORT150"),("BT_SHORT150","BTunsup_long"),("MERT_SHORT150","BTsup_long"),("BT_SHORT150","BTsup_long")]
print("n paired",len(common))
for a,b in pairs:
    d=arr(a)-arr(b)
    print(f"  {a} vs {b}: mean diff {d.mean():+.3f}  wins {np.mean(d>0):.1%}  wilcoxon p {wilcoxon(arr(a),arr(b)).pvalue:.2g}")
