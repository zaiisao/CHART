"""V2: per-frame audio-conditioned DITHER WEIGHT w_t; genuine-tempo lambda stays a single
global stiff scalar. Trained generatively (marginal likelihood, frozen frontend).

V1 (r3_generative_mixture.py) conditioned lambda_t and leaked: with w pinned globally, the only
compensation channel for per-song dither-rate variation was lambda_t, which drifted loose with
FLAT NLL (identifiability: two parameterizations of the same explanation). The per-song quantity
that truly varies is the DITHER RATE (each song's fractional grid offset), so v2 conditions w_t
(sigmoid, centered at the global 0.370) and pins lambda=93.1 clean.
...
class R3GenerativeMixture(R3ConditionedFactors):
    ...
    def per_frame_w(self, activations: torch.Tensor) -> torch.Tensor:
        """[T] dither weight: sigmoid(logit(w0) + net(x)_t) -- net zero-init -> w_t == w0."""
        x = activations.T.unsqueeze(0)
        delta = self.net(x)[0, 0]
        w0_logit = float(np.log(self.w / (1.0 - self.w)))
    ...
    def per_frame_mixture_kernel(self, activations: torch.Tensor) -> torch.Tensor:
        ...
        scores = -self.lambda_base * self.abs_ratio_dev                 # [V,V] global stiff
        ...
        return torch.logsumexp(torch.stack([
            ...
            torch.log1p(-w_t)[:, None, None] + log_exp[None]]), dim=0)
...
def main():
    ...
    for step in range(STEPS):
        ...
        if (step + 1) % 25 == 0:
            with torch.no_grad():
                spread = torch.cat([r3.per_frame_w(crops[i]) for i in idx[:4]])
            print(f"step {step+1}: nll/frame {float(loss):.4f} | w_t "
                  f"p10 {float(spread.quantile(0.1)):.3f} med {float(spread.median()):.3f} "
                  f"p90 {float(spread.quantile(0.9)):.3f}", flush=True)

    print(f"TRAINED per-frame (generative)     : beatF {val_F():.4f}", flush=True)
    torch.save(r3.state_dict(), Path(__file__).resolve().parent / "r3_generative_mixture_v2.pt")
