...
# ---- build SMC features (audio full/causal + hist), aligned to the same (song,k) pairs
def build_all(D):
    Xf, Xc, P = [], [], []
    for d in D:
        Xa, ks = song_feats(d, 300, 'full')
        Xb, ks2 = song_feats(d, 300, 'causal')
        assert ks == ks2
        if not len(ks):
            continue
        Xf.append(Xa); Xc.append(Xb); P += [(d, k) for k in ks]
    return np.concatenate(Xf, 0).astype(np.float64), np.concatenate(Xc, 0).astype(np.float64), P

Xf, Xc, P = build_all(D)
Xh, hstem = hist_feats(D)
G = gather(P)
assert len(Xh) == G['n'] == len(Xf) and (hstem == G['stem']).all()
e = G['u'] - G['u_prev']
stems = sorted(set(G['stem']))
fold_of = {s: i % 5 for i, s in enumerate(stems)}
fold = np.array([fold_of[s] for s in G['stem']])
print(f'SMC songs {len(stems)}  pairs {G["n"]}  Xfull {Xf.shape} Xcausal {Xc.shape} Xhist {Xh.shape}')
...
for nm, X in SETS.items():
    for mnm, fn in [('ridge', ridge), ('GBR', gbr)]:
        ...
        print(f'  {nm:22s} {mnm:6s} R2={r2:+.4f}  MAE={mae:.5f}  (persistence MAE {np.abs(e).mean():.5f})')
...
for tag, p in [('persistence', np.zeros_like(e)), ('hist', p_hist), ('audio-full', p_full)]:
    ...
    print(f'  next-beat |err| ms [{tag:11s}]: mean {err.mean():6.2f} median {np.median(err):6.2f} p95 {np.percentile(err,95):7.2f}')
...
for nm, (A_, B_) in TSETS.items():
    ...
    print(f'  {nm:22s} R2={r2:+.4f}  MAE={np.abs(e-p).mean():.5f}')
