...
ANCH = "/home/sogang/jaehoon/VBPM_reintegration/vbpm_anchor"
...
# ---------------------------------------------------------------------------------------------
_PHYS_JSON = json.load(open(f"{ANCH}/phys_params.json"))
_PHYS_T = json.load(open(f"{HERE}/phys_t_params.json"))     # Student-t law on the SAME D
PHYS = {
...
def kl_student_diff_dof(dof_q, mu_q, s_q, dof_p, mu_p, s_p):
    """1-sample SGVB MC estimate of KL( St(dof_q,mu_q,s_q) || St(dof_p,mu_p,s_p) ).
    rsample -> pathwise gradients reach s_q AND dof_q (Gamma implicit reparam)."""
    q = torch.distributions.StudentT(dof_q, mu_q, s_q)
    p = torch.distributions.StudentT(dof_p, mu_p, s_p)
    z = q.rsample()
...
class AnchoredVAE(SupEmisModel):
    ...
    def __init__(self, hidden=128, num_meters=4, links="residual", v_max=3.0,
                 phys="fit", p_switch=0.005, meter_prior=None, phys_law="gauss", **kw):
        ...
        # THOROUGH: physical law for the level-increment anchor (gauss = original fit_phys;
        # student = Student-t MLE on the SAME D, vbpm_thorough/phys_t_params.json)
        self.phys_law = phys_law
        self.pin_dof = False
        self.register_buffer("nu_phys", torch.tensor(float(_PHYS_T["nu"])))
        self.register_buffer("s_lv_phy_t", torch.tensor(float(_PHYS_T["scale"])))
    ...
    # ---- dof: pinnable to the physical nu; floor lowered so nu_phys=1.26 is REACHABLE ------
    def tempo_dof(self):
        if self.pin_dof:
            return self.nu_phys
        if self.phys_law == "student":
            return F.softplus(self.tempo_log_dof) + 1.0 + 1e-3
        return super().tempo_dof()
    ...
    def prior_level_scale(self, ctx_t):
        ...
        base = self.s_lv_phy_t if self.phys_law == "student" else self.s_lv_phy
        return base * torch.exp(self._res(self.prior_level_sigma, ctx_t))
...
def elbo_anchor(model, h, b, db, obs, temperature=0.5, beta=1.0, obs_w=1.0,
    ...
    for t in range(1, T):
        ...
        if model.phys_law == "student":
            A_lv = A_lv + kl_student_diff_dof(dof, mu_q, p_lv_s,
                                              model.nu_phys, mu_p, model.s_lv_phy_t)
        else:
...
def main():
    ...
    ap.add_argument("--phys_law", default="gauss", choices=["gauss", "student"],
                    help="physical level-increment law the anchor pulls toward")
    ap.add_argument("--pin_dof", action="store_true",
                    help="pin the tempo dof to nu_phys (exact-physical freeze control)")
    # --- ELBO / training ---
    ...
    print("=" * 96)
    print(f"ANCHOR[thorough phys_law] lam={lam}{' (freeze_residual = inf)' if a.freeze_residual else ''}  "
          f"links={a.links} v_max={a.v_max} phys={a.phys} phys_law={a.phys_law} "
          f"pin_dof={a.pin_dof}  elbo_obs={a.elbo_obs}  "
    ...
    model = AnchoredVAE(hidden=a.hidden, num_meters=4, links=a.links, v_max=a.v_max,
                        phys=a.phys, p_switch=a.p_switch, meter_prior=prior_vec,
                        phys_law=a.phys_law).to(DEV)
    model.freeze_residual = a.freeze_residual
    model.pin_dof = a.pin_dof
