"""ELBO + deploy paths for VARIANT A+B, RELATIVE-correction form (A2).  Copy of elbo_ab.py.
...
def transition_params(model, ctx_t, phi_prev, log_tempo_prev, level_prev, dev_prev,
    ...
    """RELATIVE-correction form (A2).  Returns the same tuple as elbo_ab.transition_params.

    WHY THIS DIFFERS FROM elbo_ab.py (the failure it repairs, measured):
      the ADDITIVE correction  advance = phi + phidot + c*tanh(.)  can cancel phidot.
      The ELBO has no term that requires the pointer to advance, so training drives the
      correction to exactly -phidot: the deploy pointer FREEZES (bars_advanced = -0.01 vs
      bars_true = 16) and the read-out degenerates into "one 2pi wrap per audio wiggle".
      That scores beat_F = 1.00 on the Dirac oracle while the bar pointer is dead.
      Additionally the ADDITIVE level correction is integrated by the OU recursion with
      gain 1/(1-a_lv) (~100), so a tiny persistent bias collapses the deploy tempo.

    A2 makes the correction MULTIPLICATIVE on the advance and drops the level correction:
        advance = phi_{t-1} + phidot_{t-1} * (1 + c * tanh(g(ctx_t, ...))),   c < 1
    so the pointer can be sped up / slowed down by up to a factor (1 +- c) -- enough to
    re-lock within a beat -- but can NEVER stop or reverse, and the tempo latent stays the
    only thing that sets the rate.  Same call site in training and in every deploy path.
    """
    tempo_prev = torch.exp(log_tempo_prev.clamp(-12.0, 6.0))
    if use_corr:                                                     # ---- (A2) ----
        g = torch.tanh(model.phase_corr(
            model._corr_feats(ctx_t, phi_prev + tempo_prev, log_tempo_prev)).squeeze(-1))
        advance = phi_prev + tempo_prev * (1.0 + model.max_phase_corr * g)
    else:
    ...
    p_lv_mu = level_anchor + a_lv * (level_prev - level_anchor)       # no level correction
