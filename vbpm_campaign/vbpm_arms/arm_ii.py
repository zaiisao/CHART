"""ARM (ii) -- ACTIVATION-ONLY (madmom-shaped).
h fed to the prior p_psi(z|h) AND posterior q_phi(z|h,b) is the frozen 2-channel beat/downbeat
ACTIVATION (h_dim=2); the observation the latent generates is that SAME activation.
The bar-pointer therefore sees strictly LESS information than the 0.811 peak-pick probe, so any
gain must come from the metrical structure itself -- the DBN-over-activation gain madmom earns.
(derived from arm_i.py; original header follows)
...
def eval_pf(merge, model, songs, obs_cache, proj, K, alpha, dev, smooth=5, seed=1234,
    ...
    for i, s in enumerate(songs):
        ...
        h = obs                      # ARM (ii): the bar-pointer sees ONLY the activation
...
def main():
    ...
    model = VB.BarPointerVAE_B(h_dim=2, hidden=a.hidden, num_meters=4,
    ...
    for step in range(1, a.steps + 1):
        ...
        if proj is not None:
            o = proj(f)
        opt.zero_grad()
        loss, info = VB.elbo_b(model, o, b, d, o, temperature=temp, beta=beta,
                               obs_w=a.obs_w)
