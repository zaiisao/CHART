...
def main() -> None:
    parser = build_standard_parser("Generate CHART phase labels for ballroom_test dataset")
    args = parser.parse_args()

    root = Path(args.root)
    data_dir = root / "ballroom_test" / "data"
    label_dir = root / "ballroom_test" / "label"
    out_dir = resolve_output_dir(root, args.out_root, args.output_mode, "ballroom_test")
    ...
    print_stats("ballroom_test", stats, out_dir)
