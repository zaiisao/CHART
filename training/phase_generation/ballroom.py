...
def main() -> None:
    parser = build_standard_parser("Generate CHART phase labels for ballroom dataset")
    args = parser.parse_args()

    root = Path(args.root)
    data_dir = root / "ballroom" / "data"
    label_dir = root / "ballroom" / "label"
    out_dir = resolve_output_dir(root, args.out_root, args.output_mode, "ballroom")
    ...
    print_stats("ballroom", stats, out_dir)
