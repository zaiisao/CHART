"""Tests for the psi-mode (conditional-prior) code paths of vbpm."""
...
def _psi_model(psi_rotations=4, d_model=8):
    _seed()
    return PsiVBPM(input_dim=4, d_model=d_model, emission="cosine",
...
# ============================================ 4. VBPM psi-mode forward


def test_psi_forward_returns_prior_anchor_and_elbo_identity():
    """Input: a padded batch through VBPM(psi=True). Asserted: the dict
...
def test_psi_state_dict_roundtrip_bit_identical_inference():
    ...
    fresh = PsiVBPM(input_dim=4, d_model=8, emission="cosine", rotations=4)
