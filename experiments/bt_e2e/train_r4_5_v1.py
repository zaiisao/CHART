...
def main():
    ...
    # PCA-whitening from train features (conditioning): 256 -> PCA_DIM, unit variance.
    # Collapses the correlated dims that made the diagonal Gaussian double-count evidence.
    stack = torch.cat([train_feats[e["stem"]] for e in train[:50]]).to(torch.float64)
    f_mean = stack.mean(0)
    cov = torch.cov((stack - f_mean).T)
    evals, evecs = torch.linalg.eigh(cov)
    top = torch.argsort(evals, descending=True)[:PCA_DIM]
    W = (evecs[:, top] / evals[top].clamp(min=1e-6).sqrt()[None]).to(torch.float32)  # [256, K]
    f_mean = f_mean.to(torch.float32)
    print(f"PCA: top-{PCA_DIM} evals explain "
          f"{float(evals[top].sum() / evals.sum()):.3f} of variance", flush=True)
    ...
    for deployed, name in ((True, "DEPLOYED"), (False, "BARE    ")):
        ...
        print(f"R4.5v1 {name}: beatF {b:.4f}  CMLt {c:.4f}  AMLt {a:.4f}  "
    ...
    torch.save({"mu": r45.mu.cpu(), "log_var": r45.log_var.cpu(),
                "f_mean": f_mean, "W": W},
               Path(__file__).resolve().parent / "r4_5_v1_gaussians.pt")
